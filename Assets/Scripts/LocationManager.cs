...
public class LocationManager : MonoBehaviour
{
    ...
    [SerializeField]
    bool UseThresholdsForPlacement;

    [SerializeField]
    bool useAccurateGroupThreshold = true;
    double[] thresholds = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55};
    double[] accurateGroupThresholds = {3, 8.5, 13.976, 25.63, 51.553, 80.822, 106.203};
    ...
    void Start()
    {
        ...
        locationIsReady = NativeGPSPlugin.StartLocation();
#endif
        arManager = this.GetComponent<ARManager>();
    }
    ...
    private void Update()
    {
        if (locationIsReady)
        {
            ...
            if (startThresholds && currentThreshold < thresholds.Length)
            {
                double dist = DistanceBetweenPointsInMeters(startLat, startLong, currLat, currLong);
                if ((useAccurateGroupThreshold && dist > accurateGroupThresholds[currentThreshold]) || (!useAccurateGroupThreshold && dist > thresholds[currentThreshold]))
                {
                    currentThreshold++;
                    StatusText.text = "Passed threshold " + currentThreshold;
                    if (UseThresholdsForPlacement)
                        arManager.NewPlanetThresholdPassed(currentThreshold);
                }
            }
        }
    }
